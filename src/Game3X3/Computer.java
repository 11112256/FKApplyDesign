      ...
      //  Same as Human but with some extra logic

        {
            ...
            System.out.println(" by 1 for player 1 and 2 for player 2");
                  int i=-1 , j=-1 ;
               for(int i_new =0 ;i_new < newboard.getgameboard().length ;i_new++) {
                   for (int j_new = 0; j_new < newboard.getgameboard().length; j_new++)
                       if (newboard.getgameboard()[i_new][j_new] == 0) {
                           i = i_new;
                           j = j_new;
                           break;
                       }
